...
<template>
  <q-page class="camera haze-bg q-pa-md relative">
    <q-card class="camera__card q-pa-lg">
      <q-form @submit.prevent="handlePublishPost(hasCameraSupport, formRef)" class="camera__wrapper" ref="formRef">
        <div class="camera__frame">
          <q-skeleton v-if="isCameraInitializing" animation="wave" class="camera__skeleton" />
          ...
          >
            <div class="camera__placeholder-wrapper">
              ...
              <span class="camera__placeholder-subtitle"> to capture the moment... </span>
            </div>
          </div>
        </div>
        ...
        <q-input
          ...
          outlined
          type="textarea"
          :rules="[(val) => !!val || 'Caption is required', (val) => val.length <= 200 || 'Maximum 200 characters']"
        />
        ...
        >
          <template #append>
            <q-spinner v-if="locationPending" color="orange" size="18px" style="margin-right: 0.45rem" />

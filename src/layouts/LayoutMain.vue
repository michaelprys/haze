  ...
  <q-layout class="main" view="lHh Lpr lFf">
    <q-header class="main__header" bordered height-hint="4rem">
      <q-toolbar class="main__container">
        <q-btn class="main__logo no-hover-effect" dense flat :ripple="false" :to="{ name: 'home' }">
          <img class="main__logo-img" alt="Haze logo" :src="Logo" width="1024" />
        </q-btn>

        <q-space />

        <q-btn class="main__title no-hover-effect" flat no-caps :ripple="false" :to="{ name: 'home' }">Haze</q-btn>
        ...
        <div class="main__header-buttons">
          ...
          <q-btn v-else class="main__header-icon" flat icon="account_circle" round :to="{ name: 'sign-in' }" />
        </div>
      </q-toolbar>
    </q-header>
    ...
    <q-footer v-if="route.meta.requiresAuth && storeAuth.isAuthenticated" bordered class="main__footer">
      <q-tabs class="main__footer-tabs" active-color="orange" align="justify" indicator-color="transparent">
